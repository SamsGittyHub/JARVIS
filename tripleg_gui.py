...
# ==========================================
# JARVIS BLUE-BLACK THEME — Iron Man Inspired
# ==========================================

THEME = {
    # Backgrounds — Deep black with subtle blue undertones (JARVIS HUD)
    "bg_dark": "#010204",
    "bg_medium": "#040810",
    "bg_light": "#08101c",
    "bg_card": "#060e18",
    "bg_input": "#030609",
    # Accent colors — JARVIS arc-reactor blue palette
    "accent_magenta": "#00b4ff",    # Primary JARVIS blue (bright)
    "accent_cyan": "#5ad4f6",       # Ice blue highlight / glow
    "accent_purple": "#1a7acc",     # Steel blue secondary
    "accent_blue": "#0080dd",       # Deep glow blue
    "accent_glow": "#00c8ff",       # Bright glow for borders/highlights
    "accent_dim": "#0a3050",        # Subtle blue for inactive borders
    # Text colors — Holographic blue-white on black
    "text_primary": "#d0e8ff",
    "text_secondary": "#7aa0c0",
    "text_dim": "#2e4a64",
    "text_bright": "#e8f4ff",       # Bright white-blue for headers
    # Bubble colors — Very dark with faint blue tint
    "user_bubble": "#051420",
    "assistant_bubble": "#06101c",
    "tool_bubble": "#041818",
    # Status colors
    "error_color": "#ff4444",
    "success_color": "#00dd99",
    "warning_color": "#ffaa22",
    # Marketplace colors
    "official_color": "#00b4ff",
    "community_color": "#5599dd",
    "specialized_color": "#ff9944",
    "custom_color": "#00ddaa",
    # HUD decorative colors
    "hud_line": "#0a2a44",          # Subtle HUD grid lines
    "hud_glow": "#00a0e0",          # HUD element glow
    "hud_border": "#0c3050",        # Panel border color
}
...
def clean_text_for_tts(text: str) -> str:
    ...
    # 0. Strip FULL tool call blocks (:::TOOL:...:::END:::) — safety net
    clean = re.sub(
        r':::TOOL:\s*\w+\s*:::\s*:::ARGS:::\s*.*?\s*:::END:::*',
        ' ', clean, flags=re.DOTALL | re.IGNORECASE
    )
    # Also strip partial/malformed tool calls
    clean = re.sub(r':::TOOL:.*?:::END:::*', ' ', clean, flags=re.DOTALL | re.IGNORECASE)
    # Strip any remaining :::TOOL:...::: or :::ARGS::: or :::END::: fragments
    clean = re.sub(r':::(?:TOOL|ARGS|END)\s*:?\s*\w*\s*:::*', ' ', clean, flags=re.IGNORECASE)
...
class JarvisSphereVisualizer(tk.Canvas):
    ...
    NUM_PARTICLES = 350       # Reduced for better performance (was 900)
    NUM_RING_PARTICLES = 50   # Particles on equator ring (was 80)
    NUM_LATITUDE_LINES = 6    # Wireframe latitude lines (was 8)
    NUM_LONGITUDE_LINES = 8   # Wireframe longitude lines (was 10)
    FPS = 20                  # Reduced for better performance (was 30)

    # Audio analysis constants
    NUM_FREQ_BANDS = 24       # Reduced FFT bands (was 32)
    AUDIO_FPS = 30            # Reduced FFT fps (was 60)
    ...
    def __init__(self, parent, **kwargs):
        ...
        self._frame = 0
        self._status_text = "Awaiting your command, sir..."
...
class StatusPanel(CTkFrame):
    ...
    def __init__(self, parent, **kwargs):
        super().__init__(parent, **kwargs)
        self.configure(fg_color=THEME["bg_medium"], height=28, corner_radius=0)

        # Left: HUD status indicator
        CTkLabel(
            self, text="◈",
            font=("Consolas", 10),
        ...
        self.status_label = CTkLabel(
            self, text="SYSTEMS ONLINE", font=("Consolas", 9),
            text_color=THEME["accent_cyan"]
        )
        self.status_label.pack(side="left", padx=4)

        # Right: model and API info
        self.api_label = CTkLabel(
            self, text=f"⟨ {CONFIG['API_URL']} ⟩",
            font=("Consolas", 8), text_color=THEME["text_dim"]
        )
        self.api_label.pack(side="right", padx=8)

        CTkLabel(
            self, text="│",
            font=("Consolas", 9), text_color=THEME["hud_line"]
        ).pack(side="right")
...
class TripleGGUI(CTk):
    ...
    def _build_ui(self):
        ...
        # JARVIS HUD-style header branding
        hud_brand = CTkFrame(chat_header, fg_color="transparent")
        hud_brand.pack(side="left", padx=10, pady=5)
        CTkLabel(
            hud_brand, text="◈",
            font=("Consolas", 14),
        ...
        CTkLabel(
            hud_brand, text="J.A.R.V.I.S.",
            font=("Consolas", 16, "bold"),
            text_color=THEME["accent_glow"]
        ).pack(side="left")
        CTkLabel(
            hud_brand, text="// TRIPLEG",
            font=("Consolas", 10),
            text_color=THEME["text_dim"]
        ...
        self.input_entry = CTkEntry(
            input_frame,
            placeholder_text="Enter command, sir...",
            font=("Consolas", 12),
            height=38,
        ...
        self.send_btn = CTkButton(
            input_frame, text="▶ SEND", width=80, height=38,
            font=("Consolas", 11, "bold"),
            fg_color=THEME["accent_blue"], hover_color=THEME["accent_glow"],
    ...
    def _send_message(self):
        ...
        self.status_panel.set_status("Working on it, sir...", THEME["warning_color"])
    ...
    def _process_message(self, user_input: str):
        ...
        try:
            for iteration in range(MAX_ITERATIONS):
                ...
                # Suppress hallucinated pre-answers when an info tool is being called
                ...
                if text and (not tool_name or tool_name not in info_tools):
                    ...
                    self.response_queue.put(("_terminal", f"[ASSISTANT] (suppressed pre-tool text: {text[:80]}...)"))
                    self.response_queue.put(("system", "🔍 Searching..."))
        ...
        try:
            while True:
                ...
                if role == "_done":
                    ...
                    self.status_panel.set_status("At your service, sir", THEME["accent_cyan"])
                    self._log_terminal("[DONE] Task complete, sir.")
                elif role == "error":
                    self.chat_panel.add_message("system", content)
                    self.status_panel.set_status("Something went wrong, sir", THEME["error_color"])
    ...
    def _new_session(self):
        ...
        self.chat_panel.clear()
        self.chat_panel.add_message("system", "New session initialized. How may I assist you, sir?")
        self.status_panel.set_status("Fresh session, sir", THEME["success_color"])
    ...
    def _toggle_voice_recording(self):
        ...
        if not self._voice_recording:
            ...
            if not self._voice_initialized:
                ...
                self.status_panel.set_status("Loading voice systems, sir...", THEME["warning_color"])
                self.chat_panel.add_message("system", "Initializing voice recognition, sir. This may take a moment on first run.")
    ...
    def _start_recording(self):
        ...
        self.status_panel.set_status("🎤 Recording... speak freely, sir", THEME["error_color"])
    ...
    def _stop_recording(self):
        ...
        self._voice_recording = False
        self.mic_btn.configure(text="⏳", fg_color=THEME["warning_color"], text_color="#000000")
        self.status_panel.set_status("Decoding your voice, sir...", THEME["warning_color"])
    ...
    def _enter_live_call(self):
        ...
        self._live_call_visualizer.set_state("idle", "Establishing connection, sir...")
        ...
        self.status_panel.set_status("📞 Live channel open, sir — hands-free", THEME["success_color"])
    ...
    def _live_call_toggle_mute(self):
        ...
        if self._live_call_muted:
            ...
            if self._live_call_visualizer:
                self._live_call_visualizer.set_state("idle", "Muted, sir")
        ...
        def _listen_loop():
            ...
            while self._live_call_active and self._live_call_listening_loop_active and not self._live_call_muted:
                try:
                    ...
                    # Filter out Whisper hallucinations (phantom words from silence/noise)
                    WHISPER_HALLUCINATIONS = {
                        "thank", "thanks", "thank you", "thank you.", "thank you!",
                        "thanks.", "thanks!", "thankyou",
                        "you", "you.", "bye", "bye.", "bye bye",
                        "okay", "okay.", "ok", "ok.",
                        "hmm", "hmm.", "hm", "uh", "um", "ah",
                        "oh", "oh.", "so", "the", "and", "i",
                        "yes", "yes.", "no", "no.", "yeah", "yep",
                        "hello", "hello.", "hi", "hi.", "hey", "hey.",
                        "right", "right.", "well", "well.",
                    ...
                    text_clean_check = text.strip().lower().rstrip(".!?,;:")
                    if text_clean_check in WHISPER_HALLUCINATIONS or len(text_clean_check) <= 2:
                        self._log_terminal(f"[LIVE CALL] Filtered Whisper hallucination: '{text.strip()}'")
                        self.voice_manager._update_state(VoiceState.IDLE, "Ready")
                        time.sleep(0.2)
                        continue
                    ...
                    # SAFETY: Strip any leaked tool protocol from response before display/TTS
                    if ":::TOOL:" in response_text or ":::ARGS:::" in response_text or ":::END:::" in response_text:
                        self._log_terminal("[LIVE CALL] WARNING: Stripping leaked tool protocol from response")
                        response_text = re.sub(r':::TOOL:.*?:::END:::*', '', response_text, flags=re.DOTALL | re.IGNORECASE).strip()
                        response_text = re.sub(r':::(?:TOOL|ARGS|END)\s*:?\s*\w*\s*:::*', '', response_text, flags=re.IGNORECASE).strip()
                        if not response_text:
                            self._log_terminal("[LIVE CALL] Response was entirely tool protocol, re-listening...")
                            time.sleep(0.3)
                            continue
            ...
            if self._live_call_active and not self._live_call_muted:
                self.after(0, lambda: self._live_call_update_ui("idle", "Awaiting your command, sir..."))
        ...
        try:
            for iteration in range(MAX_ITERATIONS):
                ...
                if tool_name:
                    ...
                    # No tool call detected by parser — but double-check for malformed tool calls
                    if ResponseParser.has_potential_tool_call(assistant_msg):
                        # Parser missed a tool call — log warning and strip it
                        self.response_queue.put(("_terminal", f"[LIVE CALL] WARNING: Potential malformed tool call detected but not parsed"))
                        # Try to extract and strip the tool call
                        final_text = re.sub(r':::TOOL:.*?:::END:::*', '', assistant_msg, flags=re.DOTALL | re.IGNORECASE).strip()
                        if not final_text:
                            # The entire response was a tool call — ask AI to respond naturally
                            self.conversation_manager.add_message(
                                "user",
                                "Your response contained a malformed tool call that couldn't be parsed. "
                                "Please provide a direct natural language answer instead."
                            )
                            continue
                    else:
    ...
    def _live_call_reset_mic(self, status_text: str = ""):
        ...
        if self._live_call_mic_btn and not self._live_call_muted:
            self._live_call_mic_btn.configure(text="🔇 Mute", fg_color=THEME["accent_cyan"], text_color="#000000")
        if self._live_call_visualizer:
            self._live_call_visualizer.set_state("idle", status_text or "Awaiting your command, sir...")
        if hasattr(self, '_live_call_status'):
