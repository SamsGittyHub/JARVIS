...
class JarvisSphereVisualizer(tk.Canvas):
    ...
    NUM_PARTICLES = 220       # Total sphere particles
...
class TripleGGUI(CTk):
            ...
            while self._live_call_active and self._live_call_listening_loop_active and not self._live_call_muted:
                try:
                    ...
                    if clean and len(clean) > 5:
                        ...
                        # Start TTS non-blocking so we can monitor mic for interruption
    ...
    def _live_call_get_ai_response(self) -> Optional[str]:
        ...
        try:
            ...
            self.response_queue.put(("_terminal", f"[LIVE CALL] Sending to AI ({len(messages)} messages)..."))
            ...
            # Parse response (strip tool calls for voice)
            parsed_text, tool_name, tool_args = self.response_parser.parse(assistant_msg)
            return parsed_text or assistant_msg

        except Exception as e:
            self._log_terminal(f"[LIVE CALL] AI error: {e}")
            return None
